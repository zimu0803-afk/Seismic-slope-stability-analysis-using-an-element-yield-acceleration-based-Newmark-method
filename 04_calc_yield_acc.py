...
# GeoStudio relative script and template file paths
...
for n in range(27):
    def run_bat():
        if not os.path.exists(BAT_PATH):
            print(f"Notice: Batch script {BAT_PATH} not found. Please deploy corresponding GeoStudio .bat file in actual working environment.")
            return
    ...
    if abs(fos_initial - TARGET) <= TOLERANCE:
        results.append({'Slip_ID': n + 1, 'Seismic_Coefficient': 0.0, 'Factor_of_Safety': round(fos_initial, 3)})
        continue
    ...
    results.append({'Slip_ID': n + 1, 'Seismic_Coefficient': round(final_seismic, 3), 'Factor_of_Safety': round(final_fos, 3)})

if results:
    df = pd.DataFrame(results)[['Slip_ID', 'Seismic_Coefficient', 'Factor_of_Safety']]
    excel_path = os.path.join(output_dir, "convergence_results.xlsx")
    df.to_excel(excel_path, index=False)
    print(f"Computation completed. Results saved to: {excel_path}")