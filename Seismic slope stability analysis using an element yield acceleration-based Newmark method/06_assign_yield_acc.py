...
def read_input_data(node_file, slip_surface_file):
    if not os.path.exists(node_file):
        raise FileNotFoundError(f"节点文件不存在: {node_file}")
    ...
    if not os.path.exists(slip_surface_file):
        raise FileNotFoundError(f"滑动面文件不存在: {slip_surface_file}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"\n跳过或处理失败: {str(e)}")
        print("请确保将测试节点文件 (nodes.xlsx) 和滑动面文件 (slip_surfaces.xlsx) 放置在 ./data 目录下。")